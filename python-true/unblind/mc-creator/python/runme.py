...
treemaker = TreeMaker(
    'MC-unblind.root',  # output file name
...
for entry in mc_treewrapper_up.entry():
    # make sure the B is a B and the K is a K and that the selection is unblind (200 MeV window around eta mass)
    # and the K prob is high (this is a strongly discriminating factor)
    if abs(mc_treewrapper_up.B_BKGCAT) == 30 and abs(mc_treewrapper_up.K_TRUEID) == 321 and mc_treewrapper_up.K_ProbNNk > 0.1 and mc_treewrapper_up.B_L0PhotonDecision_TOS == 1 and mc_treewrapper_up.B_Hlt1TrackMVADecision_TOS == 1 and mc_treewrapper_up.B_Hlt2Topo2BodyDecision_TOS == 1:
...
for entry in mc_treewrapper_down.entry():
    # make sure the B is a B and the K is a K and that the selection is unblind (200 MeV window around eta mass)
    # and the K prob is high (this is a strongly discriminating factor)
    if abs(mc_treewrapper_down.B_BKGCAT) == 30 and abs(mc_treewrapper_down.K_TRUEID) == 321 and mc_treewrapper_down.K_ProbNNk > 0.1 and mc_treewrapper_down.B_L0PhotonDecision_TOS == 1 and mc_treewrapper_down.B_Hlt1TrackMVADecision_TOS == 1 and mc_treewrapper_down.B_Hlt2Topo2BodyDecision_TOS == 1:
        treemaker.Fill({  # wants a dictionary of values
...
canvas3 = TCanvas()
that_tree = TreeWrapper('MC-unblind.root', 'DecayTree')

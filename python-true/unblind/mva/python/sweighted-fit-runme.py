...
for ii in range(int(vv+1)):

    # now to create a RooDataSet
    # create the variable to hold the data (name, title, range)
    mass = RooRealVar('mass', 'm_{gg}', ss+ii*tt-100, ss+ii*tt+100, 'MeV')
    ...
    tree = TreeWrapper('../mva/unblind_data_sweighted.root', 'DecayTree')
    for i in tree.entry():
    ...
    mean = RooRealVar('mean',  'm_{B}',  ss+ii*tt)
    sigma = RooRealVar('sigma', '#sigma',   24.4)
    ...
    Nsignal = RooRealVar('Nsignal', 'N_{Sig}', 10000, 0, 100000)
    Nbackground = RooRealVar('Nbackground', 'N_{bkg}', 10000, 0, 100000)
    ...
    print('your # signal is ' + str(Nsignal.getValV()))
    """

    mvalues.append(ss+ii*tt)
    signllvalues.append(round(sigresult.minNll()))
    bkgnllvalues.append(round(bkgresult.minNll()))
    nlldifferences.append(round(sigresult.minNll()-bkgresult.minNll()))
    significance.append(
        round(abs(sigresult.minNll()-bkgresult.minNll()) ** 0.5))
    means.append(round(mean.getValV()))
    nsignal.append(round(Nsignal.getValV()))
    errors.append(Nsignal.getError())

print(mvalues)
# print(signllvalues)
# print(bkgnllvalues)
# print(nlldifferences)
print(significance)
# print(means)
print(nsignal)
print(errors)

...
with TreeMaker("unblind_data_afterBDT01.root", "DecayTree", ['B_M', 'Kst_M', 'gamma0_P', 'gamma1_P', 'diphoton_M', 'diphoton_with_constraint_M', 'B_FD_OWNPV', 'B_P', 'B_PT', 'B_DIRA_OWNPV', 'B_ENDVERTEX_CHI2',
...
with TreeMaker("unblind_MC_afterBDT01.root", "DecayTree", ['B_M', 'Kst_M', 'gamma0_P', 'gamma1_P', 'diphoton_M', 'diphoton_with_constraint_M', 'B_FD_OWNPV', 'B_P', 'B_PT', 'B_DIRA_OWNPV', 'B_ENDVERTEX_CHI2',
                                                           'B_CONEP_100', 'B_CONEMULT_100', 'B_NC_IT_100', 'B_NEWVTXISODCHI2MASS', 'B_VTXISODCHI2MASSONETRACK',
                                                           'B_VTXISODCHI2MASSTWOTRACK', 'Kst_P', 'Kst_PT', 'K_P', 'K_PT', 'pi_P', 'pi_PT', 'Kst_IPCHI2_OWNPV',
                                                           'K_IPCHI2_OWNPV', 'pi_IPCHI2_OWNPV', 'K_ProbNNk', 'pi_ProbNNpi', 'BDT']) as outtree:
    tree = TreeWrapper(
        "unblind_MC_withBDT.root", "DecayTree")
    for entry in tree.entry():
        if tree.BDT > 0.1 and 5000 <= tree.B_M < 6500:
            outtree.Fill({
                "B_M": tree.B_M,
...
canvasx = TCanvas()
treewrapper_mc = TreeWrapper("unblind_MC_afterBDT01.root", "DecayTree")
...
if not os.path.exists('myData.root'):
    ...
    from TreeWrapper import TreeWrapper
    tree = TreeWrapper('../mva/unblind_data_afterBDT01.root', 'DecayTree')
    for i in tree.entry():

...
mass = RooRealVar('mass', 'm_{gg}', ww-100, ww+100, 'MeV')
...
for i in tree.entry():
    if mass.getMin() <= tree.diphoton_with_constraint_M < mass.getMax():
        mass.setVal(tree.diphoton_with_constraint_M)
...
mean = RooRealVar('mean',  'm_{B}',  ww, ww-100, ww+100)
...
Nsignal = RooRealVar('Nsignal', 'N_{Sig}', 1000, 0, 100000)
Nbackground = RooRealVar('Nbackground', 'N_{bkg}', 1000, 0, 100000)

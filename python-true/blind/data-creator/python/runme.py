...
sideband_treemaker = TreeMaker(
    'data-blind-sideband.root',  # output file name
    ...
    ['B_M', 'Kst_M', 'gamma0_P', 'gamma1_P', 'diphoton_M', 'diphoton_with_constraint_M', 'B_FD_OWNPV', 'B_P', 'B_PT', 'B_DIRA_OWNPV', 'B_ENDVERTEX_CHI2',
     ...
     'K_IPCHI2_OWNPV', 'pi_IPCHI2_OWNPV', 'K_ProbNNk', 'pi_ProbNNpi', 'kClass']  # list of branches
)

for entry in data_treewrapper_up.entry():
    # make sure we get high mass sideband and selection is blind (diphoton mass within 200 MeV of eta)
    # and the K prob is high (this is a strongly discriminating factor)
    if data_treewrapper_up.B_M > 5800 and abs(data_treewrapper_up.diphoton_M - 550) < 100 and data_treewrapper_up.K_ProbNNk > 0.1 and data_treewrapper_up.B_L0PhotonDecision_TOS == 1 and data_treewrapper_up.B_Hlt1TrackMVADecision_TOS == 1 and data_treewrapper_up.B_Hlt2Topo2BodyDecision_TOS == 1:
        sideband_treemaker.Fill({  # wants a dictionary of values
            ...
            'kClass': data_treewrapper_up.getEntry() % 10
        })

for entry in data_treewrapper_down.entry():
    # make sure we get high mass sideband and selection is blind (diphoton mass within 200 MeV of eta)
    # and the K prob is high (this is a strongly discriminating factor)
    if data_treewrapper_down.B_M > 5800 and abs(data_treewrapper_down.diphoton_M - 550) < 100 and data_treewrapper_down.K_ProbNNk > 0.1 and data_treewrapper_down.B_L0PhotonDecision_TOS == 1 and data_treewrapper_down.B_Hlt1TrackMVADecision_TOS == 1 and data_treewrapper_down.B_Hlt2Topo2BodyDecision_TOS == 1:
        sideband_treemaker.Fill({  # wants a dictionary of values
            ...
            'kClass': data_treewrapper_down.getEntry() % 10
        })
sideband_treemaker.close()

data_treemaker = TreeMaker(
    'data-blind-full.root',  # output file name
    'DecayTree',  # output tree name
    ['B_M', 'Kst_M', 'gamma0_P', 'gamma1_P', 'diphoton_M', 'diphoton_with_constraint_M', 'B_FD_OWNPV', 'B_P', 'B_PT', 'B_DIRA_OWNPV', 'B_ENDVERTEX_CHI2',
     'B_CONEP_100', 'B_CONEMULT_100', 'B_NC_IT_100', 'B_NEWVTXISODCHI2MASS', 'B_VTXISODCHI2MASSONETRACK',
     'B_VTXISODCHI2MASSTWOTRACK', 'Kst_P', 'Kst_PT', 'K_P', 'K_PT', 'pi_P', 'pi_PT', 'Kst_IPCHI2_OWNPV',
...
for entry in data_treewrapper_up.entry():
    # make sure selection is blind (diphoton mass within 200 MeV of eta)
    # and the K prob is high (this is a strongly discriminating factor)
    if abs(data_treewrapper_up.diphoton_M - 550) < 100 and data_treewrapper_up.K_ProbNNk > 0.1 and data_treewrapper_up.B_L0PhotonDecision_TOS == 1 and data_treewrapper_up.B_Hlt1TrackMVADecision_TOS == 1 and data_treewrapper_up.B_Hlt2Topo2BodyDecision_TOS == 1:
...
for entry in data_treewrapper_down.entry():
    # make sure selection is blind (diphoton mass within 200 MeV of eta)
    # and the K prob is high (this is a strongly discriminating factor)
    if abs(data_treewrapper_down.diphoton_M - 550) < 100 and data_treewrapper_down.K_ProbNNk > 0.1 and data_treewrapper_down.B_L0PhotonDecision_TOS == 1 and data_treewrapper_down.B_Hlt1TrackMVADecision_TOS == 1 and data_treewrapper_down.B_Hlt2Topo2BodyDecision_TOS == 1:
        data_treemaker.Fill({  # wants a dictionary of values
            'B_M': data_treewrapper_down.B_M,
            'Kst_M': data_treewrapper_down.Kst_M,
            'gamma0_P': data_treewrapper_down.gamma0_P,
...
that_tree.Draw('B_M', 'abs(B_M - 5300) < 300')
canvas3.Modified()
canvas3.Update()

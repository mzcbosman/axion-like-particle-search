...
treemaker = TreeMaker(
    'MC-reduced.root',  # output file name
...
for entry in mc_treewrapper_up.entry(Nentries=1000):
    if abs(mc_treewrapper_up.B_BKGCAT) == 30 and abs(mc_treewrapper_up.K_TRUEID) == 321:
...
for entry in mc_treewrapper_down.entry(Nentries=1000):
    if abs(mc_treewrapper_down.B_BKGCAT) == 30 and abs(mc_treewrapper_down.K_TRUEID) == 321:
        treemaker.Fill({  # wants a dictionary of values
...
canvas3 = TCanvas()
that_tree = TreeWrapper('MC-reduced.root', 'DecayTree')

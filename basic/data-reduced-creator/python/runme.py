...
sideband_treemaker = TreeMaker(
    'data-reduced-sideband.root',  # output file name
    'DecayTree',  # output tree name
    ['B_M', 'Kst_M', 'gamma0_P', 'gamma1_P', 'B_FD_OWNPV', 'B_P', 'B_PT', 'B_DIRA_OWNPV', 'B_ENDVERTEX_CHI2',
...
for entry in data_treewrapper_up.entry(Nentries=5000):
    if data_treewrapper_up.B_M > 5800:
...
for entry in data_treewrapper_down.entry(Nentries=5000):
    if data_treewrapper_down.B_M > 5800:
        sideband_treemaker.Fill({  # wants a dictionary of values
...
data_treemaker = TreeMaker(
    'data-reduced-full.root',  # output file name
    'DecayTree',  # output tree name
    ['B_M', 'Kst_M', 'gamma0_P', 'gamma1_P', 'B_FD_OWNPV', 'B_P', 'B_PT', 'B_DIRA_OWNPV', 'B_ENDVERTEX_CHI2',
     'B_CONEP_100', 'B_CONEMULT_100', 'B_NC_IT_100', 'B_NEWVTXISODCHI2MASS', 'B_VTXISODCHI2MASSONETRACK',
     'B_VTXISODCHI2MASSTWOTRACK', 'Kst_P', 'Kst_PT', 'K_P', 'K_PT', 'pi_P', 'pi_PT', 'Kst_IPCHI2_OWNPV',
...
for entry in data_treewrapper_up.entry(Nentries=2500):
...
for entry in data_treewrapper_down.entry(Nentries=2500):
    data_treemaker.Fill({  # wants a dictionary of values
        'B_M': data_treewrapper_down.B_M,
        'Kst_M': data_treewrapper_down.Kst_M,
        'gamma0_P': data_treewrapper_down.gamma0_P,
...
canvas2 = TCanvas()
that_tree = TreeWrapper('data-reduced-sideband.root', 'DecayTree')
...
canvas3 = TCanvas()
that_tree = TreeWrapper('data-reduced-full.root', 'DecayTree')
# Draw the original B mass in a 100 MeV window around the peak
that_tree.Draw('B_M')
canvas3.Modified()
canvas3.Update()

...
for entry in data_treewrapper_up.entry():
    ...
    if data_treewrapper_up.B_M > 5800 and abs(data_treewrapper_up.diphoton_M - 550) < 100:
...
for entry in data_treewrapper_down.entry():
    # make sure we get high mass sideband and selection is blind (diphoton mass within 200 MeV of eta)
    # and the K prob is high (this is a strongly discriminating factor)
    if data_treewrapper_down.B_M > 5800 and abs(data_treewrapper_down.diphoton_M - 550) < 100:
        sideband_treemaker.Fill({  # wants a dictionary of values
...
for entry in data_treewrapper_up.entry():
    ...
    # and the K prob is high (this is a strongly discriminating factor)
    if abs(data_treewrapper_up.diphoton_M - 550) < 100:
...
for entry in data_treewrapper_down.entry():
    # make sure selection is blind (diphoton mass within 200 MeV of eta)
    # and the K prob is high (this is a strongly discriminating factor)
    if abs(data_treewrapper_down.diphoton_M - 550) < 100:
        data_treemaker.Fill({  # wants a dictionary of values
            'B_M': data_treewrapper_down.B_M,
            'Kst_M': data_treewrapper_down.Kst_M,
            'gamma0_P': data_treewrapper_down.gamma0_P,
